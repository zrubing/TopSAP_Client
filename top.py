...
class AutoLoginTopSap():
    def __init__(self, host, port, username, password, ocr, server_addr, server_port):
        logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
        self.host = host
        self.port = port
        self.username = username
        self.password = password
        self.code_url = f"https://localhost:7443/api/v1/get_gid?serverAddr={server_addr}&serverPort={server_port}&vpn_version=ngvone&auth_protocol=0&auth_port=10443&data_port=10443&data_protocol=0&cert_type=0&proxyType=&proxyAddr=&proxyPort=&proxyUser=&proxyPwd=&proxyDomain=&rnd=0.21450320730512473"
...
server_port = config.get(env, 'serverPort')

print("start")
auto_login = AutoLoginTopSap(host, port, username, password, ocr, server_addr, server_port)
auto_login.listen()
